...
**I-66** — Hecho (David, con fotos de interruptores NEMA y DIN y captura del gabinete): el gabinete ponía el valor del interruptor como texto a la derecha de la celda. Corrección: `Layout/Interruptor.razor`, duotono como `Icono.razor` (trazo de 1.5 px, relleno del acento al 16 %), genérico, sin marca ni logotipo. NEMA: de lado, polos apilados cada 34 px (renglón de 30 + hueco de 4) unidos por una sola manija con el valor calado; en el gabinete, cada polo queda frente a su espacio. DIN: de pie, módulos de 18 lado a lado, bornes arriba y abajo, «C16» en el frente; hecho pero sin usar, porque el único gabinete es NEMA. Valores: los de la serie (`SerieDeInterruptores`). Propuesta en el canvas «Iconos de termomagnético». Prueba: navegador, claro y oscuro, circuitos de 1, 2 y 3 polos — bloque de 30/64/98 px con icono de 24/58/92 px; sin desborde a 390 px; sin errores en la consola salvo la API de GitHub sin red. **Segunda versión** (David: «esa línea para 2 y 3 polos, horrible»; «deja fuera el rótulo»; «analiza bien cómo se ve un interruptor», con fotos del QO y de riel DIN): el NEMA pasa a un QO genérico visto de frente —pinza a la barra, cuerpo, cuello que sale por la tapa, manija en su ranura, ventana de disparo, zapata con tornillo por polo— con UNA manija ancha en 2 y 3 polos, como el QO real, y sin rótulo; el amperaje vuelve a ir como texto junto al icono. En el gabinete va acostado con la pinza hacia la barra del centro: en los nones a la derecha, en los pares a la izquierda y primero. Mide 34n − 10 × 46 px. Se quita el DIN hasta que haya gabinete de riel. Prueba: navegador, claro y oscuro — bloques de 30/64/98 px con icono de 24/58/92 px, a 7 px del borde en los pares; sin desborde a 390 px. Commit: `f850580`. **Tercera versión** (David, con el icono de flaticon y fotos del QO de 1, 2 y 3 polos: «todos tienen una sola manija; el de dos polos la tiene en el polo derecho y el de 3 en el central; la cubierta no tiene nada en los polos sin manija»; y «muy anchos»): módulo por polo arriba (pinza) y abajo (zapata con tornillo), tapa ancha al centro con UNA manija de ancho normal y la ventana de disparo en el único polo, el derecho o el central; lisa en los demás. Largo de 62 (2.6 veces el polo), no 46. En el gabinete, 62 px de ancho: las descripciones se cortan más. Prueba: navegador, claro y oscuro — bloques de 30/64/98 px con icono de 24/58/92 px; sin desborde a 390 px. Commit: `3a70aea`. **Cuarta versión** (David: «yo los veo igual de anchos»): alargar no bastó, el polo seguía de 34, amarrado al renglón del gabinete. Ahora 20 por polo y 62 de largo (casi 1 : 3, como el QO), centrado en su celda; los polos ya no calzan uno a uno con los renglones. La rejilla lleva `grid-auto-rows: minmax(30px, auto)`: sin el icono que la estiraba, un interruptor de 3 polos se encogía a 82 px. Prueba: navegador — bloques de 30/64/98 px con icono de 20/40/60 px; sin desborde a 390 px. Commit: `ba636bd`.

**I-67** — Hecho (David, en el canvas «Gabinete · reacomodos de la tarjeta de circuito»: de cinco propuestas eligió B, tapa muerta y directorio, y pidió dibujar las barras en multifilar; «relleno = fase, contorno = circuito»): el gabinete eran tarjetas con pastillas de fase apiladas y el nombre se cortaba. Corrección: `Layout/InteriorDelGabinete.razor`. Dos o tres barras (1F-3H, 2F-3H, 3F-3H, 3F-4H): barras verticales del color de su fase con su `.fase-rect` arriba; cada espacio sale de su barra con una conexión horizontal, un punto de unión (4.5 px con anillo) y su número en un recuadro de contorno; nones a la izquierda, pares a la derecha; interruptores acostados junto a la tapa y el directorio (nombre y amperes) afuera; espacios libres con conexión punteada y tapón. Una barra (1F-2H): barra horizontal, cada espacio baja con su número, interruptor de frente, amperes y nombre en vertical; se recorre de lado si no cabe. Un renglón = un polo del QO a 1.2 (24 px), así cada conexión llega a su polo (`Interruptor.Escala`). Barra A negra en claro y gris claro en oscuro (`--barra-a/b/c`). El gabinete pasa a 1.5 del ancho de `.cierre` y las tres columnas empiezan en 1540 px (con 1.5 el resumen se salía hasta 19 px entre 1401 y 1470); con la tarjeta de menos de 560 px el directorio baja a una lista (consulta de contenedor). Prueba: navegador — 3F-4H, 1F-3H y 1F-2H en claro y oscuro: 24/12/8 números, barras ABC/AB/A, un icono por circuito; sin desborde de tarjetas de 1200 a 1920 px ni a 390 px; sin errores en la consola. Visto al probar, fuera de este cambio: a 1000 px la barra superior (tema y GitHub) desborda la página 129 px.
